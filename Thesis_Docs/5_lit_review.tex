...
We will now discuss the convolutional neural network structure utilised by Ghafoorian et al. \cite{GhafoorianM.2017Dml3}. This model forms the basis of our adjusted location-independent model. This model consists of two phases. The first phase generates candidates using a \textsc{cnn}. The second phase reduces false-positives using a 3D \textsc{cnn} and candidate location data.
...
The input data captures the same point at three different resolutions. The original scales for each sample are 32$\times$32$\times$5, 64$\times$64$\times$5 and 128$\times$128$\times$5. These samples are reduced to dimensions 32$\times$32$\times$5. In total, there were $3.85\times10^5$ samples for training, and $3.5\times10^4$ for validation. 

Each resolution is fed through its own 3D \textsc{cnn}, as shown in Figure \ref{litrev-ghafoorian_model2fig}. Each \textsc{cnn} branch contains six convolution layers, which have 64, 64, 128, 128, 256 and 256 filters of sizes 3$\times$3$\times$2, 3$\times$3$\times$2, 3$\times$3$\times$1, 3$\times$3$\times$1, 3$\times$3$\times$1 and 3$\times$3$\times$1 respectively.
