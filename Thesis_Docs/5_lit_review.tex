...
All weights are intialised using the He method. Training is conducted by stochastic gradient descent with Adam Optimiser. Each training batch has a sample size of 128. A decaying learning rate is used, from $5\times10^{-4}$ to $10^{-6}$ by the last epoch.
...
The input data captures the same point at three different resolutions. The original scales for each sample are 32$\times$32$\times$5, 64$\times$64$\times$5 and 128$\times$128$\times$5. These samples are reduced to dimensions 32$\times$32$\times$5. In total, there were 385K samples for training, and 35K for validation. 
