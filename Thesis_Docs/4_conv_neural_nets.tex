...
\todo[inline]{Explain the need for zero padding, stride, pooling and relu layers. Convolution involves a large number of variables, many of which are redundant when identifying important features. These methods work to reduce the number of redundant variables, speed up processing and provide flexibility in network design.}

During regular convolution, the output arrays are of reduced dimension to the input. We now discuss zero padding and stride, techniques that control how the subimages are chosen and the resulting image dimensions.
