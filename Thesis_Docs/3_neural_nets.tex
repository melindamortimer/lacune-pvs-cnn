...
Neural networks have become increasingly popular with the advancement of computing power. They have the ability to detect and utilise predictors that may not be easily employed using other methods. Given enough data, computational power, and training time, neural network models exhibit high accuracy in solving complex problems. In some cases, recorded accuracies have been found to be higher than that of humans \cite{HeK2016Drlf} and have been proven successful in \textsc{mri} classification tasks \cite{DouQ.2016ADoC, Yokoyama2007b}.
...
Activation functions $\sigma(\cdot)$ are applied just before neuron output. Without these functions, the network can be reduced to a linear combination of inputs. Activation functions serve to introduce nonlinearity into the model.
