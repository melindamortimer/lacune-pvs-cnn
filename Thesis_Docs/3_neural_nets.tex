...
\begin{example}
A logical \textsc{and} gate of two inputs can be expressed as a neuron. Let \textsc{true} be encoded to 1 and \textsc{false} be encoded to 0, so that $x_i \in \{0,1\}$ for $i=1,2$. Set $\mathbf{w} = (1, 1)^\intercal$ and $b = -1.5$. Set the activation function to be the step function $\sigma(z) = 1$ for $z > 0$ and $\sigma(z) = 0$ otherwise. The resulting neuron has output
\[
	a = \begin{cases}
		1 & x_1 + x_2 - 1.5 > 0\\
		0 & else
...
The set of weights $W$ and set of biases $B$ are chosen such that they minimise some \textit{cost function} $C(W,B)$ with respect to $(W,B)$. However, since the number of variables can be very large, it is not always feasible to analytically minimise $C(W,B)$. Instead, we can approximate this minimisation via the Gradient-Descent algorithm (see Section \ref{nnets-graddesc}). This algorithm is run until the cost function falls within some tolerance $\tau$, or reaches the maximum number of iterations $n_T$.

\section{Notation}\label{nnets-not}

\begin{tabular}{rl}
$\mathbf{w}^\ell_j$ & vector of weights used to compute the $j$-th neuron of the $\ell$-th layer\\
$w_{ji}^\ell$ & weight attributed to the $i$-th neuron of the $(\ell-1)$-th layer.
\end{tabular}
