...
For isolated lacunes, Yokoyama et al. used multiple-phase binarisation. This method involves using mean pixel values in the MRI to develop thresholds used for binarisation \textbf{(Fig 3 \cite{Yokoyama2007})}. Then for lacunes that are next to white matter hyperintensities, images are manipulated and candidate lacunes are found by considering their area, circularity and gravitational centre.

% Fig. 3 from Yokoyama 2007
%\begin{figure}
%	\includegraphics[scale=\texwidth]{•}
%\end{figure}
...
Candidate lacunes then satisfy
\begin{align*}
	19 \le A \le 200 \\
	0.45 \le C \\
	(g_x - c_x)^2 + (g_y - c_y)^2 < 12000,
\end{align*}
where $(c_x, c_y)$ is the center of a region of interest that has a high volume of lacunes, such as the basal ganglia.
