...
The basic neural network node is the \textit{neuron}.
...
\begin{itemize}
	\item Initialise the weights and biases.
	\item Calculate the gradient of the cost function.
	\item Shift all the weights and biases in the direction of the greatest decrease in cost.
\end{itemize}
\item Two issues: no guarantee that the minimum is global, and overfitting.
\end{itemize}
\todo[inline]{Reduce content}
\end{frame}
...
\item 
\item False-positives can be further reduced by applying the false-positive reduction model by Ghafoorian et al. \cite{GhafoorianM.2017Dml3}.
\item 
\end{itemize}
